...
function extractFilename(headers) {
  const match = headers.match(/Content-Disposition:\s*attachment;\s*filename="([^"]+)"/i);
...
async function main() {
  ...
  if (!inputFile) {
    console.error("Usage:");
    console.error("  node mime2folder.js <input.mime> [output-folder]");
    process.exit(1);
  }
  ...
  if (!boundary) throw new Error("Could not find MIME boundary.");
  ...
  for (const part of parts) {
    ...
    await writeFileSafe(resolvedOutput, file.filename, file.content);
